...
fs.readFile('./tmp/client_secret.json', function processClientSecrets(err, content) {
    if (err) {
    ...
    authorize(JSON.parse(content), insertIvent);
...
function insertEvent(auth) {
    getExcelData('./tmp/file.xlsm', function (fileDataToObject) {

        var calendar = google.calendar('v3');

        var events = [];

        for (var i = 0; i < fileDataToObject.length; i++) {
...
function readExcel() {
    ...
    file.mv('./tmp/file.xlsm', function (err) {
        if (err)
            console.error("Error to load file");
